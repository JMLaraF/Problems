...
bool insideCheck(poligono &v , point p)
{
	ld t = 0;
	ld sn;
//	cout << v.size() << " PZ\n";
	for(int i = 1 ; i <= v.size() ; i++)
	{
	//	cout << line(v[i-1] , v[i%v.size()]).dist_Segpto(p) << " <D\n";
		if(v[i-1] == p || v[i%v.size()] == p)
			return true;
		if(line(v[i-1] , v[i%v.size()]).dist_Segpto(p) <= ERR)
			return true;
//			cout << (v[i-1] - p).x << " , " << (v[i-1] - p).y << " | " << (v[i%v.size()] - p).x << " , " <<  (v[i%v.size()] - p).y << '\n';
		sn = (((v[i-1] - p)^(v[i%v.size()] - p))/((v[i-1] - p).mgt()*(v[i%v.size()] - p).mgt()));
//			cout << sn << " <SN\n";
		if(fabs(sn - 1) <= ERR)
			t += PI/2;
		else if(fabs(sn + 1) <= ERR)
			t -= PI/2;
		else
			t += 2*(asin(((v[i-1] - p)^(v[i%v.size()] - p))/((v[i-1] - p).mgt()*(v[i%v.size()] - p).mgt())));
//			cout << t << " <T\n";
...
{_C
	...
	while(!(p == 0 && l == 0 && c == 0))
	{
		...
		{
			cin >> pc.x >> pc.y;
			bool pt = insideCheck(pp , pc);
			bool rt = insideCheck(pl , pc);
			cout << pt << " [A] " << rt << '\n';
/*			if(p > 2 && pp.size() < 3 && pp.size() > 0)
				if(pp[0] == pc || pp[1] == pc || line(pp[0] , pp[1]).dist_Segpto(pc) <= ERR)
					pt = true;
			if(l > 2 && pl.size() < 3 && pl.size() > 0)
				if(pl[0] == pc || pl[1] == pc || line(pl[0] , pl[1]).dist_Segpto(pc) <= ERR)
					rt = true;	
